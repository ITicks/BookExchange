...
public class DataSourceStatistiche extends AbstractDataSource {
	...
	private String countInserzioni = "SELECT COUNT(*) AS numInserzioni FROM Inserzioni";
	
	/** Conto il numero delle inserzioni da validare */
	private String countInserzioniDaValidare = "SELECT COUNT(*) AS numInsDaValid FROM Inserzioni"
	...
	public StatisticheModel getStatistiche(){
		...
		try {
			if (rs.next()) statistiche.setNur(rs.getInt("numUtenti"));
		...
		try {
			if (rs.next()) statistiche.setNcb(rs.getInt("numCorrettori"));
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		} 
		...
		try {
			if (rs.next()) statistiche.setNiv(rs.getInt("numInserzioni"));
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		...
		try {
			if (rs.next()) statistiche.setNidv(rs.getInt("numInsDaValid"));
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		} 
